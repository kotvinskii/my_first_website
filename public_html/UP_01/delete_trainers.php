...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_delete'])) {
    if (!empty($trainer_id) && $trainer) {
        try {
            ...
            $errors['db'] = 'Ошибка удаления: возможно, запись связана с командами. ' . $e->getMessage();
        }
    }
}
?>
...
<div align="center">
    ...
    <?php if (empty($trainer_id)): ?>
        <h2>Форма удаления тренера</h2>
        <form method="POST">
            <table style="border: none;">
                <tr style="border: none;">
                    <td style="border: none;" align="right">Введите ID тренера для удаления:</td>
    ...
    <?php else: ?>
        ...
        <table>
            ...
            <tr><td class="label">Опыт (лет):</td><td><?= htmlspecialchars($trainer['experience_years']) ?></td></tr>
            ...
            <tr><td class="label">Фото:</td><td><?= htmlspecialchars($trainer['trainer_image']) ?></td></tr>
        </table>
        <form method="POST">
