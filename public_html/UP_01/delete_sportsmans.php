...
$errors = [];
$competition_id = trim($_POST['athlete_id'] ?? $_GET['athlete_id'] ?? '');
$competition = null;
...
if (!empty($competition_id)) {
    ...
    if (!$competition) {
        $errors['athlete_id'] = "Удаление с ID $competition_id не найдено.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_delete'])) {
    if (!empty($competition_id) && $competition) {
        try {
            ...
            header("Location: sportsmans.php?msg=deleted");
            exit;
...
<head>
    ...
    <title>Удаление спортсменов</title>
...
<body>
    <div align="center">
        ...
        <?php if (empty($competition_id)): ?>
            <h2>Форма удаления спортсмена</h2>
            <form method="POST">
                <table style="border: none;">
                    <tr style="border: none;"><td style="border: none;" align="right">Введите ID спорстмена для поиска:</td>
        ...
        <?php else: ?>
            ...
            <h3>Информация о спорстмене №<?= htmlspecialchars($competition_id) ?></h3>
            
            <table>
                <tr><td class="label">ID спортсмена:</td><td><?= htmlspecialchars($competition['athlete_id']) ?></td></tr>
                <tr><td class="label">Фамилия:</td><td><?= htmlspecialchars($competition['last_name']) ?></td></tr>
                <tr><td class="label">Имя:</td><td><?= htmlspecialchars($competition['first_name']) ?></td></tr>
                <tr><td class="label">Отчество:</td><td><?= htmlspecialchars($competition['middle_name']) ?></td></tr>
                <tr><td class="label">Дата проведения:</td><td><?= htmlspecialchars($competition['birth_date']) ?></td></tr>
                <tr><td class="label">Ранг:</td><td><?= htmlspecialchars($competition['rank']) ?></td></tr>
                <tr><td class="label">ID клуба:</td><td><?= htmlspecialchars($competition['club_id']) ?></td></tr>
                <tr><td class="label">Достижения:</td><td><?= htmlspecialchars($competition['achievements']) ?></td></tr>
                <tr><td class="label">ID клуба:</td><td><?= htmlspecialchars($competition['club_id']) ?></td></tr>
                ...
                <tr><td class="label">Фото:</td><td><?= htmlspecialchars($competition['athlet_image']) ?></td></tr>
            </table>

            <form method="POST">
                <!-- Передаем ID и скрытый триггер удаления -->
                <input type="hidden" name="athlete_id" value="<?= htmlspecialchars($competition_id) ?>">
                ...
                <a href="sportsmans.php" class="btn btn-secondary">Отмена</a>
            </form>
        <?php endif; ?>
    </div>
</body>
