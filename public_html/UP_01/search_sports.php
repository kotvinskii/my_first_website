...
<div align="center">
    ...
    <?php if (empty($sport_id)): ?>
        <h2>Форма поиска вида спорта</h2>
        <form method="POST">
            <table style="border: none;">
                <tr style="border: none;">
                    <td style="border: none;" align="right">Введите ID спорта для поиска:</td>
    ...
    <!-- ЭТАП 2: Вывод найденной информации -->
    <?php else: ?>
        <h3>Информация о виде спорта №<?= htmlspecialchars($sport_id) ?></h3>
        <table>
            <tr><td class="label">ID спорта:</td><td><?= htmlspecialchars($sport['sport_id']) ?></td></tr>
            ...
            <tr><td class="label">Олимпийский вид:</td><td><?= $sport['olympic_sport'] ? 'Да' : 'Нет' ?></td></tr>
        </table>
        <a href="?" class="btn btn-secondary">Искать другой спорт</a>
    <?php endif; ?>
</div>
