...
if (!empty($result_id)) {
    ...
    if (!$result) {
        $errors['result_id'] = "Результат соревнований с ID $result_id не найден.";
        $result_id = ''; // Сбрасываем ID, чтобы вернуть пользователя на шаг ввода
    }
}
?>
<!DOCTYPE html>
...
<body>
    <div align="center">
        ...
        <?php if (empty($result_id)): ?>
            <h2>Форма нахождения результата соревнований</h2>
            <form method="POST">
                <table style="border: none;">
                    <tr style="border: none;">
                        <td style="border: none;" align="right">Введите ID результата для поиска:</td>
        ...
        <?php else: ?>
            <h3>Информация о результате №<?= htmlspecialchars($result_id) ?></h3>
            <table>
                <tr><td class="label">ID Результата:</td><td><?= htmlspecialchars($result['result_id']) ?></td></tr>
                <tr><td class="label">ID Соревнования:</td><td><?= htmlspecialchars($result['competition_id']) ?></td></tr>
                <tr><td class="label">ID Спортсмена:</td><td><?= htmlspecialchars($result['athlete_id']) ?></td></tr>
                <tr><td class="label">Занятое место:</td><td><?= htmlspecialchars($result['place']) ?></td></tr>
                <tr><td class="label">Значение результата:</td><td><?= htmlspecialchars($result['result_value']) ?></td></tr>
                <tr><td class="label">Единица измерения:</td><td><?= htmlspecialchars($result['result_unit']) ?></td></tr>
                <tr><td class="label">Медаль:</td><td><?= htmlspecialchars($result['medal']) ?></td></tr>
                <tr><td class="label">Статус рекорда:</td><td><?= htmlspecialchars($result['record_status']) ?></td></tr>
            </table>
            <a href="?" class="btn btn-secondary">Искать другой результат</a>
        <?php endif; ?>
    </div>
</body>
</html>
