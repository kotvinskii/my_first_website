...
if (!empty($organizer_id)) {
    ...
    if (!$organizer) {
        $errors['organizer_id'] = "Организатор с ID $organizer_id не найден.";
        $organizer_id = ''; // Сбрасываем ID, чтобы вернуть пользователя на шаг ввода
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Поиск организатора</title>
    <style>
...
<body>
    <div align="center">
        ...
        <?php if (empty($organizer_id)): ?>
            <h2>Форма нахождения организатора</h2>
            <form method="POST">
                <table style="border: none;">
                    <tr style="border: none;">
                        <td style="border: none;" align="right">Введите ID организатора для поиска:</td>
        ...
        <?php else: ?>
            <h3>Информация об организаторе №<?= htmlspecialchars($organizer_id) ?></h3>
            <table>
                <tr><td class="label">ID Организатора:</td><td><?= htmlspecialchars($organizer['organizer_id']) ?></td></tr>
                <tr><td class="label">Название:</td><td><?= htmlspecialchars($organizer['name']) ?></td></tr>
                <tr><td class="label">Контактное лицо:</td><td><?= htmlspecialchars($organizer['contact_person']) ?></td></tr>
                <tr><td class="label">Телефон:</td><td><?= htmlspecialchars($organizer['phone']) ?></td></tr>
                <tr><td class="label">Email:</td><td><?= htmlspecialchars($organizer['email']) ?></td></tr>
                <tr><td class="label">Адрес:</td><td><?= htmlspecialchars($organizer['address']) ?></td></tr>
            </table>
            <a href="?" class="btn btn-secondary">Искать другого организатора</a>
        <?php endif; ?>
    </div>
</body>
</html>
