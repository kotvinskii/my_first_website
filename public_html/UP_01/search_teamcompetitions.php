...
if (!empty($team_competition_id)) {
    ...
    $team_competition = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$team_competition) {
        $errors['team_competition_id'] = "Командный результат с ID $team_competition_id не найден.";
        $team_competition_id = ''; // Сбрасываем ID, чтобы вернуть пользователя на шаг ввода
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Поиск командного результата</title>
    <style>
...
<body>
    <div align="center">
        <h1>Найти командные результаты</h1>
        ...
        <?php if (empty($team_competition_id)): ?>
            <h2>Форма нахождения командного результата</h2>
            <form method="POST">
        ...
        <?php else: ?>
            ...
            <table>
                <tr><td class="label">ID Командного результата:</td><td><?= htmlspecialchars($team_competition['team_competition_id']) ?></td></tr>
                <tr><td class="label">ID Соревнования:</td><td><?= htmlspecialchars($team_competition['competition_id']) ?></td></tr>
                <tr><td class="label">ID Команды:</td><td><?= htmlspecialchars($team_competition['team_id']) ?></td></tr>
                <tr><td class="label">Занятое место:</td><td><?= htmlspecialchars($team_competition['place']) ?></td></tr>
                <tr><td class="label">Результат:</td><td><?= htmlspecialchars($team_competition['result']) ?></td></tr>
            </table>
            <a href="?" class="btn btn-secondary">Искать другой командный результат</a>
        <?php endif; ?>
    </div>
</body>
</html>
